...
const GroupTh = ({ children, span }: { children: React.ReactNode; span: number }) => (
  <th colSpan={span} className="border border-slate-900 bg-[#1e1b4b] text-white px-1 py-1 text-[9px] font-bold uppercase tracking-wide">
    {children}
  </th>
);

const SubTh = ({ children }: { children: React.ReactNode }) => (
  <th className="border border-slate-900 bg-slate-100 px-1 py-1 text-[8px] font-semibold leading-tight align-middle">
    {children}
  </th>
);

const Val = ({ children }: { children: React.ReactNode }) => (
  <td className="border border-slate-900 px-1 py-1.5 text-[9px] font-bold text-center align-middle">
    {children}
  </td>
);
...
export function ShepherdReportPrint({ data }: { data: PrintData | null }) {
  ...
  return (
    <div id="shepherd-report" className="print-only bg-white text-slate-900 p-4" style={{ fontFamily: "Arial, sans-serif" }}>
      {/* Titre */}
      <h1 className="text-center text-[15px] font-black uppercase tracking-wide mb-3">
      ...
      <table className="w-full border-collapse table-fixed">
        <thead>
          ...
          <tr>
            ...
            <SubTh>Évangélisation de groupe</SubTh>
      ...
      <table className="w-full border-collapse table-fixed mt-2">
        <thead>
          <tr>
            ...
            <th className="border border-slate-900 bg-slate-100 px-1 py-1 text-[8px] font-semibold w-[40%]">
              Le livre à étudier
            </th>
          </tr>
        </thead>
        <tbody>
          <tr>
            ...
            <td className="border border-slate-900 px-2 py-1.5 text-[9px] font-bold">{form.meditated_book || ""}</td>
          </tr>
        </tbody>
      </table>
      ...
      <table className="w-full border-collapse mt-2">
        <thead>
          <tr>
            <th className="border border-slate-900 bg-[#1e1b4b] text-white px-2 py-1 text-[9px] font-bold uppercase w-1/2">
              Observations
            </th>
            <th className="border border-slate-900 bg-[#1e1b4b] text-white px-2 py-1 text-[9px] font-bold uppercase w-1/2">
              Autres observations
        ...
        <tbody>
          <tr className="align-top">
            <td className="border border-slate-900 px-2 py-1.5 text-[9px]">
            ...
            <td className="border border-slate-900 px-2 py-1.5 text-[9px] whitespace-pre-wrap">
      ...
      {/* NB + attestation */}
      <div className="mt-2 text-[8px] leading-snug text-slate-700">
      ...
      <p className="mt-2 text-[9px] italic leading-snug">
      ...
      <div className="mt-6 text-[9px] flex justify-end">
        <div className="text-center">
          <div className="border-t border-slate-900 w-40 pt-1">Signature du berger</div>
        </div>
      </div>
    </div>
  );
